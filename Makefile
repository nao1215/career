...
GO_PKGROOT = ./...
LDFLAGS    = -s -w -X github.com/nao1215/career/cmd.Version=$(VERSION)
...
coverage: ## Combine unit + self-hosted E2E coverage into coverage.out / coverage.html (scratch under .coverage/)
...
demo: build ## Regenerate the README demo GIF from docs/demo.tape (needs vhs)
	...
	vhs docs/demo.tape
	-rm -f cv.pdf japanese-resume.pdf work-history.pdf resume.yaml
	@echo 'Regenerated image/demo.gif'

clean: ## Clean build and test artifacts
	-rm -rf $(APP) coverage.out coverage.html *.pdf .coverage
