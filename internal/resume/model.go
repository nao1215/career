...
type Profile struct {
	...
	NameKana  string  `yaml:"name_kana"`  // ふりがな
...
// Career holds the 職務経歴書-only fields.
type Career struct {
	Summary        string          `yaml:"summary"`        // 職務要約
	Skills         []string        `yaml:"skills"`         // 活かせる経験・知識・技術
	Histories      []CareerHistory `yaml:"histories"`      // 職務経歴
	Certifications []string        `yaml:"certifications"` // 資格
	Publications   []string        `yaml:"publications"`   // 出版・登壇など
	SelfPR         string          `yaml:"self_pr"`        // 自己PR
}

// CareerHistory is one employer block in the 職務経歴.
type CareerHistory struct {
	Company  string    `yaml:"company"`  // 会社名
	Period   string    `yaml:"period"`   // 在籍期間, e.g. "2022年1月 - 2025年2月"
	Role     string    `yaml:"role"`     // 役職, optional
	Summary  string    `yaml:"summary"`  // 事業内容・概要, optional
	Projects []Project `yaml:"projects"` // 担当プロジェクト
}

// Project is one assignment within a CareerHistory.
type Project struct {
	Title       string   `yaml:"title"`       // 案件名・テーマ
	Period      string   `yaml:"period"`      // 期間, optional
	Role        string   `yaml:"role"`        // 役割・規模, optional
	Description string   `yaml:"description"` // 業務内容
	Tech        []string `yaml:"tech"`        // 使用技術
}
